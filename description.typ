...
#set table(align: center, stroke: rgb("add8e6") + 1pt, inset: 0pt, columns: 3)
  ...
  (
    (
      ...
      [- $sum_(t=2)^(k+1) S_i$ 는 루트 노드를 제외한 트리의 크기이므로 $N-1$입니다.]
    ),
    ...
    (
      ...
      [- 여기에서 $S_i>=1$이므로, 절댓값을 그대로 빼내서 식을 정리할 수 있습니다.]
    ),
    (
      [- $sum_(i=2)^(k+1)|D_i-B_i| &= (S_2 - 1) + (S_2+S_3-2) + dots + (sum_(j=2)^(k)S_j-(k-1))#linebreak() &= (k-1)S_2 + (k-2)S_3 + ... + S_k - sum_(i=1)^(k-1)\i$],
      [- 위 식이 최대가 되도록 $S_t$를 배분하는 것은 $S_2=N-k$, 나머지는 전부 한 개씩 나누는 것입니다.],
      [- $S_2$를 루트로 하는 서브트리에서 노드를 배치하는 형태는 $sum_(i=1)^N|D_i-B_i|$를 최대화하는 데 영향을 미치지 않습니다.],
    ...
    (
      [- 식을 펼쳐 계산하면 $k=(N-1)/2$인 경우에 최대가 되며, 언급한 대로 트리를 구축해주면 됩니다.],
    )
  ),
