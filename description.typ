...
#let descriptions = (
  ...
  (
    ...
    (
      ...
      [- 이분 탐색에 깃대를 활용하기 위해 깃대를 정렬하는 데 $cal(O)(M log M)$입니다.],
      [- 하나의 밑변 길이에 대해서, $R$ 이하인 최대 넓이를 구하는 데 $cal(O)(log M)$이 듭니다.],
      [- $cal(O)(M log M + N^2 log M)$ 은 충분히 통과할 수 있습니다.]
    ),
  ),
  ...
  (
    ...
    (
      [- 아래쪽으로 누적합을 구해준 뒤, 오른쪽 아래 대각선 방향으로 누적합을 구해나가도 정답을 구할 수 있습니다.],
      [],[],
      [
        
        #let c1 = rgb(240,229,235);
        #let c2 = rgb(212,236,220)
        #let c3 = rgb(252,249,218)
        #let c4 = rgb(252,223,215)
#set table(align: center, stroke: rgb("add8e6") + 1pt, inset: 0pt, columns: 3)
        #align(center)[
        #grid(columns: 5)[
          
          #align(center + horizon)[
        #table()[#cell(1, fill: c1)][#cell(2,fill: c2)][#cell(3,fill: c3)][#cell(5, fill: c1)][#cell(2,fill: c2)][#cell(1,fill: c3)][#cell(0, fill: c1)][#cell(2, fill: c2)][#cell(4,fill: c3)]]
   ...
   (
    ...
    (
      [- 정점을 세 개로 나누어 아래와 같은 그래프 모형을 생각할 수 있습니다.],
    ...
    (
      [- 더 쉬운 풀이로, 단순하게 오른쪽으로 한 번, 왼쪽으로 한 번, 다시 오른쪽으로 한 번 DP를 훑어도 해결할 수 있습니다.],
      [- 아직 나에게 오는 경로가 없거나 이미 학교에 도착한 경우, 칸에 숫자가 $0$이 쓰인 경우 등을 잘 처리해야 합니다.],
  ...
  (
    (
      ...
      [- 위 세 가지 규칙을 지키지 않는다면 #mono("NO", color:PALE_RED)를 출력해야 합니다.],
      [- 그 이외의 경우에는 모두 가능할까요?],
    ),
    (
      [- 몇 가지 관찰을 해 봅시다.],
    ...
    (
      ...
      [- 길이가 $S$인 구간에서 $i<j$ 이면서 $A_i < A_j$인 $(i, j)$ 쌍의 개수를 구해야 합니다.],
    ),
    (
      [- 구간 내 원소에 대해서 모든 쌍을 찾는 방법을 먼저 생각해 봅시다.],
      [- 하나의 구간에 대해 $cal(O)(N^2)$이 걸리므로, 모든 구간에서 계산한다면 $cal(O)(N^3)$으로 제한시간 내에 풀 수 없습니다.],
    ...
    (
      ...
      [- 해당 구간의 연산을 매 구간마다 반복하지 않으면 효율적으로 풀 수 있습니다.],
    ),
    (
  ...
  (
    (
      [- BFS의 순회 순서대로 정점에 번호를 붙이겠습니다. 정점을 적절히 배치해서 순회 순서의 차를 최대화해야 합니다.],
      [- 루트 바로 아래에 $k$개의 정점이 존재한다고 합시다.],
      [- 트리의 $i(1 <= i <= N)$번 정점을 루트로 하는 서브트리의 크기를 $S_i$라고 합시다.],
      [- $sum_(i=2)^(k+1) S_i$ 는 루트 노드를 제외한 트리의 크기이므로 $N-1$입니다.]
    ),
    ...
    (
      ...
      & + |sum^(k)_(j=2)S_j+2-(k+1)| #linebreak() &=|S_2-1| + |S_2+S_3-2| + dots + |sum^(k)_(j=2)S_j-(k-1)|$],
      [- 여기에서 서브트리의 크기는 1 이상이므로, 절댓값 내부의 식을 빼내서 식을 정리할 수 있습니다.]
    ),
    (
      [- $sum_(i=2)^(k+1)|D_i-B_i| &= (S_2 - 1) + (S_2+S_3-2) + dots + (sum_(j=2)^(k)S_j-(k-1))#linebreak() &= (k-1)S_2 + (k-2)S_3 + ... + S_k - sum_(i=1)^(k-1)\i$],
      [- 위 식이 최대가 되도록 $S_i$를 배분하는 것은 $S_2=N-k$, 나머지는 $1$로 두는 것입니다.],
      [- $2$번 정점을 루트로 하는 서브트리에서 노드를 배치하는 형태는 $sum_(i=1)^N|D_i-B_i|$를 최대화하는 데 영향을 미치지 않습니다.],
      [- 따라서 $k$값 중, 문제의 정답을 최대화하는 값을 찾아야 합니다.],
    ),
    (
      [#v(-2em)#align(center)[#image("images/bfsdfs.png", width: 50%)]],
      [- 순회 순서의 차를 보면, $N-k-1$개에 해당하는 #math.op("BFS") 순서는 $k+1$씩 밀려납니다.],
      [- 반대로, $k-1$개의 #math.op("DFS") 순서는 $N-k+1$씩 밀립니다.],
      [- 따라서 $(k-1) times (N-k+1) + (k+1) times (N-k-1)$의 최댓값을 구해야 합니다.]
    ),
    (
      [- 식을 펼쳐 계산하면 $k=N/2$인 경우에 최대가 되며, 언급한 대로 트리를 구축해주면 됩니다.],
    )
  ),
  ...
  (
    ...
    (
      [- 다음과 같은 배열을 봅시다.],
      [#align(center)[#table(columns:6)[#cell(2, color:PALE_RED)][#cell(1,color:PALE_RED)][#cell(5,color:blue)][#cell(3,color:blue)][#cell(6,color:blue)][#cell(4,color:blue)]]],
      [- 이 배열이 정렬되기 위해서는 어떻게 해야 할 지 생각해 봅시다.],
      [- 각 원소가 자신의 자리로 돌아가기 위해서, 현재 적혀있는 수에 해당하는 칸으로 간선을 그어 봅시다.],
      [- 배열에서 사이클을 분리해낼 수 있습니다. 사이클의 종류는 총 세 가지입니다.],
      [- 왼쪽 절반 또는 오른쪽 절반으로만 이루어져 있는 사이클, 교차하는 사이클],
      [- 각각을 $L$, $R$, $M$ 사이클이라고 정의합시다.]
    ),
    (
      [- $L$ 사이클과 $R$ 사이클은 그 자체로 교환이 불가합니다.],
      [- $M$ 사이클로 만들어서 정렬하는 과정이 필요합니다.],
      [- $1$회 교환을 통해서 원소가 한 개 늘어난 $M$ 사이클을 만들 수 있습니다.],
      [- 이때, $L$과 $R$ 사이클이 둘 다 존재한다면, 두 사이클의 원소를 서로 교환해서 하나의 $M$ 사이클을 만들 수 있습니다.],
      [- 이는 각 사이클을 $M$ 사이클로 만들었을 때보다 교환을 $1$번 적게 할 수 있으므로, 가능하다면 $L$, $R$ 사이클 간 교환을 통해 $M$ 사이클을 만드는 것이 이득입니다.],
    ),
    (
      [- 이제 $M$ 사이클을 올바르게 배치하는 데 드는 비용을 알아봅시다.],
      [- 크기가 $2$인 $M$ 사이클을 정렬하는 데에는 $1$번의 교환이면 충분합니다.],
      [- 크기가 $i$인 $M$ 사이클을 정렬하는 데 걸리는 비용이 $k$라고 합시다.],
      [- 크기가 $i+1$인 $M$ 사이클을 정렬할 때, 세 개 이상의 원소를 동시에 교환할 수 있는 방법이 없으므로 $1$번의 교환을 통해 크기가 $i$인 $M$ 사이클을 만드는 것이 최선입니다.],
      [- $i+1$ 개의 원소를 가지는 $M$ 사이클은 $k+1$번 교환을 통해 정렬할 수 있습니다.],
      [- $i=2$일 때 $k=1$이므로, 크기가 $N$인 $M$ 사이클은 $N-1$번의 교환을 통해 정렬할 수 있습니다.]
    ),
    (
      [- 따라서 $L$, $R$, $M$ 사이클의 개수를 각각 구해준 뒤, $L$, $R$ 사이클 쌍들을 $M$ 사이클들로 만들어 줍시다.],
      [- 남는 $L$, $R$ 사이클은 크기가 $1$ 증가한 $M$ 사이클이 됩니다.],
      [- 기존 $M$ 사이클은 각 사이클의 원소의 개수보다 $1$ 작은 횟수로 정렬이 가능합니다. ],
      [- $M$ 사이클을 구성하는 왼쪽/오른쪽 원소의 개수가 같은 쪽으로 가도록 교환하면 $M$ 사이클을 유지하면서 크기를 $1$씩 줄여나갈 수 있습니다.],
      [- 사이클의 개수, 종류, 구성하는 원소 등을 조합해 위에서 설명한 대로 답을 도출할 수 있습니다.],
      [- 시간복잡도는 $cal(O)(N)$입니다.]
    )
  ),
